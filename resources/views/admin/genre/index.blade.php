...
<body>
            ...
            @foreach ($genre as $item)
                @if ($item->page == $page && $item->genre == $genreparam)
                    ...
                    <tr>
                        ...
                        <td>{{ $item->page }}</td>
                        <td>{{ $item->genre }}</td>
                        <td style="width: 750px;">{{ $item->animeId }}</td>
                        <td>
                        ...
                        <td>
                            <a href="/en-db-genre/{{ $item->id }}/edit" class="btn btn-warning btn-sm"><svg
                            ...
                            <button type="button" class="btn btn-danger btn-sm" data-bs-toggle="modal"
                                data-bs-target="#staticBackdrop1{{ $item['animeId'] }}"><svg
                                    xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                            ...
                            <!-- Modal -->
                            <div class="modal fade" id="staticBackdrop1{{ $item['animeId'] }}"
                                data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
                                aria-labelledby="staticBackdropLabel" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered">
                                    <div class="modal-content">
    ...
    @if (0 == $i)
        <center>
            <p style="padding-top: 30vh;">"No Anime have been added yet."</p>
            <a class="btn btn-primary btn-sm" href="{{ route('genrePreInsert') }}">Single anime Insert</a>
            @if (Auth::check() && Auth::user()->theme === 'light')
                <a class="btn btn-dark btn-sm" href="{{ route('prepopulateGenre') }}">Populate</a>
            @else
                <a class="btn btn-light btn-sm" href="{{ route('prepopulateGenre') }}">Populate</a>
            @endif
        </center>
    @else
        @if ($page == 1)
            <nav aria-label="Page navigation example">
                <ul class="pagination justify-content-center">
                    ...
                    <li class="page-item"><a class="page-link"
                            href="{{ route('admin-genre-manage') }}?genre={{ $genreparam }}&page={{ $next }}">{{ $next }}</a>
                    </li>
                    <li class="page-item"><a class="page-link"
                            href="{{ route('admin-genre-manage') }}?genre={{ $genreparam }}&page={{ $next2 }}">{{ $next2 }}</a>
                    </li>
                    <li class="page-item">
                        <a class="page-link"
                            href="{{ route('admin-genre-manage') }}?genre={{ $genreparam }}&page={{ $next }}">Next</a>
        ...
        @elseif($page > 2)
            <nav aria-label="Page navigation example">
                <ul class="pagination justify-content-center">
                    <li class="page-item">
                        <a class="page-link"
                            href="{{ route('admin-genre-manage') }}?genre={{ $genreparam }}&page={{ $prev }}">Previous</a>
                    </li>
                    <li class="page-item"><a class="page-link"
                            href="{{ route('admin-genre-manage') }}?genre={{ $genreparam }}&page={{ $prev2 }}">{{ $prev2 }}</a>
                    </li>
                    <li class="page-item"><a class="page-link"
                            href="{{ route('admin-genre-manage') }}?genre={{ $genreparam }}&page={{ $prev }}">{{ $prev }}</a>
                    </li>
                    <li class="page-item active"><a class="page-link" href="#">{{ $page }}</a></li>
                    <li class="page-item"><a class="page-link"
                            href="{{ route('admin-genre-manage') }}?genre={{ $genreparam }}&page={{ $next }}">{{ $next }}</a>
